...
<body class="gray-bg">
					...
					<form id="form_user" method="post" action="reserveStu.htm">
						...
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							...
							<tr>
								...
								<td>入校时间：</td>
								   <td>
                     <input placeholder="入校时间" id="entrytime" name="entrytime" class="laydate-icon form-control layer-date"/>
               					  </td>
							</tr>
							<tr>
								<td>代课老师：</td>
	...
	<!-- 新增老师对话框 -->
	<div class="modal fade" id="modal_user_addteacher" role="dialog" aria-labelledby="modal_user_addteacher" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-body">
					<form id="form_addteacher" method="post" action="reserveTeacher.htm">
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							<tr>
								<td>老师姓名：</td>
								<td><input type="text" id="name" name="name"
							...
							<tr>
								...
								<td>入校时间：</td>
								   <td>
                     <input placeholder="入校时间" id="entrytimes" name="entrytime" class="laydate-icon form-control layer-date"/>
               					  </td>
							</tr>
						...
						<div class="modal-footer">
							<button type="button" class="btn btn-primary"  id="submit_form_addteacher_btn">保存</button>
							<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
						</div>
					</form>
	...
	$(function () {
	    ...
	    //添加部门
	    var validator = $("#form_addteacher").validate({
    		submitHandler: function(form){
   		      $(form).ajaxSubmit({
   		    	dataType:"json",
   		    	success: function (data) {
   		    		...
   		    		if(data.success && !data.errorMsg ){
   		    			validator.resetForm();
   		    			$('#modal_user_addteacher').modal('hide');
   		    			$("#btn_search").click();
   		    		}else{
   		    			$("#select_message").text(data.errorMsg);
   		    			$("#alertmod_table_user_mod").show();
	    ...
	    $("#submit_form_addteacher_btn").click(function(){
	    	$("#form_addteacher").submit();
	    });
	});
	...
	var ButtonInit = function () {
	    ...
	    oInit.Init = function () {
	    	...
	    	$("#btn_addteacher").click(function(){
	    		$("#form_addteacher").resetForm();
	    		$('#modal_user_addteacher').modal({backdrop: 'static', keyboard: false});
				$('#modal_user_addteacher').modal('show');
	        });
			    	    ...
			    	    series : [
			    	        {
			    	            ...
			    	            center: ['50%', '60%'],
			    	            data:[
			    	                {value:res.lb, name:'刘邦'},
			    	                {value:res.wck, name:'舞长空'},
			    	                {value:res.hhs, name:'胡汉三'}
			    	            ],
			    	            itemStyle: {
			    	                emphasis: {
			    	                    shadowBlur: 10,
